...
class Response extends HTTPResponse{
    ...
    public function check(){
        ...
        if($this->buildChecksum()!==$this->_params["control"]) throw new ariuspayGatewayAruispayException("Checksum is invalid",500);
            ...
            switch($k){
                ...
                default:
                    if(!isset($this->_params["{$k}"])){
                        throw new ariuspayGatewayAruispayException("Control field {$k} is not set.",403);
                    }
                    $str.=$this->_params["{$k}"];
                    break;
            }
    ...
    protected function checksum($str){
        $str = preg_replace("/[\r\n]+/","",$str);
        $res = sha1($str);
        ariuspay::debug("Make checksum SHA1:what:[{$str}]:get:{{$res}}");
        return $res;
    }
}
?>
